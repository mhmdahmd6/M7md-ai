...
// --- استدعاء المفتاح من متغيرات البيئة لضمان الأمان ---
...
const M7mdAIInterface = () => {
  ...
  // وظيفة تحويل الملف لـ Base64 ليفهمه Gemini API
  ...
  // --- محرك التعامل مع Gemini API مع معالجة الأخطاء ---
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (activeTool === 'prompt') {
        const prompt = `بصفتك خبير Prompt Engineering، حول هذه الفكرة لبرومبت إنجليزي دقيق لمولدات الصور: ${userInput}`;
    ...
    <div className="min-h-screen bg-[#020617] text-white font-sans selection:bg-blue-500/30 pb-20" dir="rtl">
      ...
      <nav className="sticky top-0 z-50 bg-[#020617]/90 backdrop-blur-xl border-b border-blue-900/30 px-6 py-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className="text-xl font-black bg-gradient-to-l from-blue-400 to-white bg-clip-text text-transparent italic tracking-tighter uppercase">M7MD AI</span>
        </div>

        <div className="flex gap-2 bg-black/40 p-1 rounded-xl border border-white/5">
          <button onClick={() => {setActiveTool('image'); setShowResult(false);}} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold transition-all ${activeTool === 'image' ? 'bg-blue-600 text-white shadow-blue-glow' : 'text-gray-400 hover:text-blue-400'}`}>
            <ImageIcon size={16} /> مولد الصور
          </button>
          <button onClick={() => {setActiveTool('prompt'); setShowResult(false);}} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold transition-all ${activeTool === 'prompt' ? 'bg-indigo-600 text-white shadow-purple-glow' : 'text-gray-400 hover:text-indigo-400'}`}>
            <Wand2 size={16} /> هندسة النصوص
          </button>
        </div>
      </nav>

      <main className="max-w-6xl mx-auto p-4 md:p-10 animate-fade-up text-right">
        <div className="flex flex-col lg:flex-row gap-8 items-start">
            ...
            {isLoading && (
              <div className="px-4">
                ...
                <div className="w-full h-1.5 bg-blue-900/20 rounded-full overflow-hidden">
                  <div className="h-full bg-blue-500 transition-all duration-300 shadow-blue-glow" style={{ width: `${progress}%` }}></div>
                </div>
              </div>
            )}

            {showResult && (
              <div className="bg-blue-500/5 border border-blue-500/10 p-6 rounded-[2rem] animate-fade-up shadow-inner">
          ...
          <div className="w-full lg:flex-[1] space-y-4">
            <div className="bg-[#0f172a]/40 p-5 rounded-[1.5rem] border border-white/5">
              <label className="text-[10px] font-bold text-gray-500 mb-4 block text-center uppercase tracking-widest">الأبعاد (Aspect Ratio)</label>
              ...
              <div className="flex flex-wrap gap-2">
                {files.map((item) => (
                  <div key={item.id} className="relative w-14 h-14 group">
                    ...
                    <button onClick={() => removeFile(item.id)} className="absolute -top-1 -left-1 bg-red-600 rounded-full p-1 opacity-0 group-hover:opacity-100 transition-all shadow-lg text-white">
                      <X size={10}/>
                    </button>
                  </div>
                ))}
            ...
            <button 
              ...
              className={`w-full py-4 rounded-2xl font-black text-white flex items-center justify-center gap-3 transition-all ${isLoading ? 'opacity-40 cursor-not-allowed' : 'hover:scale-[1.02] active:scale-95 shadow-2xl'} ${activeTool === 'image' ? 'bg-blue-600 shadow-blue-glow' : 'bg-indigo-600 shadow-purple-glow'}`}
            >
              {isLoading ? <Loader2 className="animate-spin" size={20} /> : <Sparkles size={18} fill="white" />}
              <span className="text-sm font-black uppercase tracking-widest">{isLoading ? 'جاري التحليل...' : activeTool === 'image' ? 'تحليل وتوليد' : 'هندسة النص'}</span>
            </button>
          </div>
        </div>
      </main>
